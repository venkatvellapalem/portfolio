...
export default function BlogContent({ content }: BlogContentProps) {
  ...
  useEffect(() => {
    ...
    result = result.replace(/^## (.+)$/gm,
      '<h2 style="font-size:1.35rem;font-weight:700;color:#e2e8f0;margin:2.5rem 0 1rem;padding-bottom:0.5rem;border-bottom:1px solid #1e293b;font-family:\'Sora\',sans-serif;">$1</h2>')
    result = result.replace(/^# (.+)$/gm,
      '<h1 style="font-size:1.75rem;font-weight:700;color:#e2e8f0;margin:2.5rem 0 1rem;font-family:\'Sora\',sans-serif;">$1</h1>')
    ...
    result = result.replace(/^[\*\-] (.+)$/gm,
      '<li style="padding-left:0.5rem;margin:0.35rem 0;color:#e2e8f0;list-style:none;"><span style="color:#22c55e;margin-right:0.5rem;">▸</span>$1</li>')
    ...
    result = result
      ...
      .map((block) => {
        ...
        return `<p style="color:#94a3b8;line-height:1.8;margin:1rem 0;font-size:1rem;">${trimmed.replace(/\n/g, '<br />')}</p>`
      })
      .join('\n')
